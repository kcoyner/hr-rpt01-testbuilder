...
describe('MasterCard', function() {
  ...
  /*  -- Refactored by using expect above --
...
describe('Discover', function() {
  ...
  for (let prefix = 644; prefix <= 649; prefix++) {
    (function(prefix) {
      let ccNum = prefix.toString() + '4567890123456789';
  ...
  for (let prefix = 644; prefix <= 649; prefix++) {
    (function(prefix) {
      let ccNum = prefix.toString() + '4567890123456';
...
/*
TODO:  refactor Maestro so the four prefix's are in a nested loop with len
*/

function generateCCNumber (len){
...
describe('Maestro', function() {
  ...
  for (let len = 12; len <= 19; len++) {
    (function(len) {
      let prefix = '5018';
      let ccNum = prefix + generateCCNumber(len - 4);
...
// 6759 and a length of 16, 18, or 19.
  var expect = chai.expect;
  let prefixs = [4903, 4905, 4911, 4936, 6333, 6759];
  let ccLengths = [16, 18, 19];
  prefixs.forEach(prefix => {
    ccLengths.forEach(len => {
      (function(len) {
        let ccNum = prefix + generateCCNumber(len - 4);
        it('has a prefix of ' + prefix + ' and a length of ' + len.toString(), function() {
  ...
  prefixs = [564182, 633110];
  prefixs.forEach(prefix => {
    ccLengths.forEach(len => {
      (function(len) {
        let ccNum = prefix + generateCCNumber(len - 6);
        it('has a prefix of ' + prefix + ' and a length of ' + len.toString(), function() {
          expect(detectNetwork(ccNum)).to.equal('Switch');
        });
      })(len);
...
describe('China UnionPay', function() {
  var expect = chai.expect;
  let prefixs = [624, 625, 626];
  let ccLengths = [16, 17, 18, 19];
  prefixs.forEach(prefix => {
    ccLengths.forEach(len => {
      (function(len) {
        let ccNum = prefix + generateCCNumber(len - 3);
        it('has a prefix of ' + prefix + ' and a length of ' + len.toString(), function() {
          expect(detectNetwork(ccNum)).to.equal('China UnionPay');
        });
      })(len);
    });
  });

  prefixs = [6282, 6283, 6284, 6285, 6286, 6287, 6288];
  prefixs.forEach(prefix => {
    ccLengths.forEach(len => {
      (function(len) {
        let ccNum = prefix + generateCCNumber(len - 4);
